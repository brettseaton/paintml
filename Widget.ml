...
let make_controller (v: 'a) : 'a value_controller =
  ...
  let funcs = {contents = []} in
  {
  add_change_listener = (fun (f) -> 
                         funcs.contents <- (funcs.contents @ [f]));
  get_value = (fun () -> value.contents);
  change_value = (fun (h) -> value.contents <- h;
                 List.iter (fun f -> f h ) funcs.contents)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
 ...
 (** Creating widget with label and selection indicator*)
 let l = fst (label s) in 
 let c = fst (canvas (10,10) (fun g -> if controller.get_value() then
                                       (Gctx.draw_line g (0,0) (10,10);
                                       Gctx.draw_line g (0,10) (10, 0))
                                       else ()))
 in
 let cl = hpair c l in
 (** wrapping widget with notifier to recognize mouseclick*)
 let (cb, nc) = notifier cl in
 let checkbox_click () : unit = 
                      controller.change_value (not (controller.get_value()))
 in
 nc.add_event_listener (mouseclick_listener checkbox_click);
 (cb , controller) 
...
(** Radio button manager with the ability to add buttons and listeners  *)
type 'a radio_button_manager = {
 add_button : 'a -> string -> widget;
 add_listener : ('a -> unit) -> unit
}

(** Creates radio button manager with appropriate functions *)
let create_radio_button_manager (init: 'a) : 'a radio_button_manager =
 let controller = make_controller init in
 {
  ...
  add_button = (fun v s ->
      (** Creating widget with label and selection indicator*)
      let l = fst (label s) in 
      let c = fst (canvas (10,10) (fun g -> if (controller.get_value() = v)
                                         then (Gctx.draw_line g (0,0) (10,10);
                                         Gctx.draw_line g (0,10) (10, 0))
                                         else ()))
      in
      let cl = hpair c l in
      (** wrapping widget with notifier to recognize mouseclick*)
      let (rb, nc) = notifier cl in
      let radiobutton_click () : unit = 
                        controller.change_value (v)
      in
      nc.add_event_listener (mouseclick_listener radiobutton_click);
      rb
    );
  (** Adds listener*)
  add_listener = controller.add_change_listener
 }
