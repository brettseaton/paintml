...
type shape = 
  | Line of {color: Gctx.color; tck: int; p1: point; p2: point}
  | Points of {color: Gctx.color; tck: int; points: point list}
  | Ellipse of {color: Gctx.color; tck: int; p1: point; p2: point}
...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      | Line l -> draw_line (with_params g l.color l.tck) l.p1 l.p2
      | Points ps -> draw_points (with_params g ps.color ps.tck) ps.points
      | Ellipse ell -> let x1 = fst ell.p1 in
                       let x2 = fst ell.p2 in
                       let y1 = snd ell.p1 in
                       let y2 = snd ell.p2 in
      
              draw_ellipse (with_params g ell.color ell.tck) 
                           (((x1 + x2) / 2), ((y1 + y2) / 2)) 
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
          ...
          | PointMode -> 
            paint.preview <- Some (Points {color= paint.color; tck = paint.tck;
                                    points= [p]})

          | EllipseStartMode -> 
            paint.mode <- EllipseEndMode p

          | _ -> () (*Ignore in all other modes*)
    ...
    | MouseDrag ->
      ...
      (begin match paint.mode with 
          | LineEndMode p1 ->
            ...
            paint.preview <- Some (Line {color=paint.color; tck = paint.tck; 
                                         p1=p1; p2=p});
          ...
          | EllipseEndMode p1 -> 
            paint.preview <- Some (Ellipse {color=paint.color; tck = paint.tck;
                                            p1=p1; p2=p});
      ...
      (begin match paint.mode with 
          | LineEndMode p1 ->
            (* The paint_canvas was waiting for the second click of a line,
              ...
              to waiting for the first click. *)
            Deque.insert_tail
              (Line {color=paint.color; tck = paint.tck; p1=p1; p2=p}) 
              paint.shapes;
            paint.preview <- None;
          ...
          | PointMode -> 
            (begin match paint.preview with
            | Some (Points ps) -> Deque.insert_tail (Points ps) paint.shapes;
            | _ -> ()
            end);
            paint.preview <- None
          ...
          | EllipseEndMode p1 -> 
            Deque.insert_tail
              (Ellipse {color=paint.color; tck = paint.tck; p1=p1; p2=p}) 
              paint.shapes;
            paint.preview <- None;
...
let mode_button_manager = create_radio_button_manager (LineStartMode)
let w_line = mode_button_manager.add_button LineStartMode "Line"
let w_point = mode_button_manager.add_button PointMode "Point"
let w_ellipse = mode_button_manager.add_button EllipseStartMode "Ellipse"

;; mode_button_manager.add_listener (fun m -> paint.mode <- m)
...
(** This function runs when the Point button is clicked.
     It sets the paint mode to allow the user to draw points*)

;; nc_point.add_event_listener (mouseclick_listener (set_mode PointMode))
...
;; nc_ellipse.add_event_listener(mouseclick_listener(set_mode EllipseStartMode))
...
let mode_toolbar : widget = 
 vlist [border w_undo; spacer; hlist [w_line; spacer; w_point; spacer;
        w_ellipse; spacer]]
...
(** Creates the checkbox for thickness. *)
let (tck_cb, tck_vc) = checkbox false "Thickness"

(** Listener function to change thickness after checking box*)
let change_tck (tck : bool) : unit =
   paint.tck <- (if tck then 4 else 2)

(** Registering the above listener. *)
;; tck_vc.add_change_listener (change_tck)
...
(** The color selection toolbar. Contains the color indicator and
   ...
   (color_button magenta); spacer;  tck_cb] 
